...
import {PRODUCT_TOPIC, PRODUCT_STOCK_UPDATE_QUEUE_ROUTING_KEY } from '../../../config/rabbitmq/queue.js';
...
export function sendMessageToProductStockUpdateQueue(message) {
    amqp.connect(RABBIT_MQ_URL, (error, connection) => {
        ...
        connection.createChannel((error, channel) => {
            ...
            channel.publish(PRODUCT_TOPIC, 
                PRODUCT_STOCK_UPDATE_QUEUE_ROUTING_KEY,
                Buffer.from(jsonStringMessage)
            );
            console.info(`Message was sent successfull`);
        });
    });
}